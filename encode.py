...
def get_units(dataset, model, sampled_paths, gamma=0.2, layer=7, save=False):
    ...
    for wav_path in tqdm(sampled_paths, desc="Getting units"):
        ...
        if model == "dusted":
            ...
            word, word_id = get_hubert_units(
                dataset, wav, hubert, wav_path, flags, wav_df, word_id, save
            )
...
def get_hubert_units(
    dataset, wav, hubert, wav_path, flags, wav_df, word_id, save=False
):
    words = []
    ...
    for w in range(max(wav_df["word_id"])):
        ...
        new_word.add_encoding_by_flags(units, flags, discrete=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Process dataset configurations for sampling and feature extraction."
    )
    ...
    parser.add_argument(
        "--audio_ext", type=str, default=".flac", help="File extension for audio files."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Number of samples to take from the dataset (-1 for all).",
    )

    parser.add_argument(
        "--n_jobs", type=int, default=8, help="Number of parallel jobs to use."
    )
    parser.add_argument(
        "--gamma", type=float, default=0.2, help="Parameter for get_units function."
    )
    parser.add_argument(
        "--layer", type=int, default=7, help="Parameter for get_units function."
    )
